...
for i in range(0,N+1):
    a[i] = Lambda     # Diagonal principal
    d[i] = -alfa      # Diagonal secundário superior
    c[i] = -alfa      # diagonal secundária inferior

# Construção do LADO DIREITO "B" do sistema tridiagonal
b[1] = 2 * alfa * u_left + (1 - 2 * alfa) * u_ant[1] + alfa * u_ant[2]
for i in range(2, N - 1):
    b[i] = alfa * u_ant[i - 1] + (1 - 2 * alfa) * u_ant[i] + alfa * u_ant[i + 1]
b[N - 1] = alfa * u_ant[N - 2] + (1 - 2 * alfa) * u_ant[N - 1] + 2 * alfa * u_right
...
for t in range(1, Passo_Tempo_Total + 1):
    ...
    # Construção do NOVO LADO DIREITO "B" do sistema
    b[1] = 2 * alfa * u_left + (1 - 2 * alfa) * u_ant[1] + alfa * u_ant[2]
    for i in range(2, N - 1):
        b[i] = alfa * u_ant[i - 1] + (1 - 2 * alfa) * u_ant[i] + alfa * u_ant[i + 1]
    b[N - 1] = alfa * u_ant[N - 2] + (1 - 2 * alfa) * u_ant[N - 1] + 2 * alfa * u_right
...
plt.title('Equação de difusão de calor (Crank-Nicolson)')
plt.show()
